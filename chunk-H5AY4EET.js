...
<p data-ke-size="size16">&nbsp;</p>\r
`}];var F=class l{constructor(e){this.router=e}history$=new u([]);get currentMenu(){return this.history$.value.at(-1)}home=i(o({},z),{command:e=>this.navigateTo(e)});flatList=[];items=[];initialized=!1;initMenu(){if(!this.initialized){this.items=[{label:"Games",items:[...p]},{label:"JSON Tools",items:[...d]},{label:"Other Tools",items:[...h]}];let e=[...f].reverse(),t=w.map(r=>i(o({},r),{count:e.filter(n=>n.category===r.category).length}));this.flatList.push(c),this.flatList.push(...e,...p,...d,...h,...t,...m);let a={label:"Trading",items:[...m]};this.items.push(a);let s={label:"Blog",items:[c,...t]};this.items.push(s),this.flatList.forEach(r=>r.command=n=>this.navigateTo(n)),this.initialized=!0}}getUrl(e){return e.routerLink}getArticleUrl(e){return e.routerLink}navigateTo(e){let t=e.item;this.router.navigateByUrl(t.routerLink)}navigateToArticle(e){let t=i(o({},e),{routerLink:this.getArticleUrl(e)});this.router.navigateByUrl(t.routerLink)}getRelevantArticles(e){let t=[];if(e){let a=this.flatList.filter(r=>!r.count&&r.category===e.category),s=a.indexOf(e);[-2,-1,1,2].forEach(r=>{let n=s+r;n>=0&&n<a.length&&t.push(a[n])})}return t}getMenuByUrl(e){return this.flatList.find(t=>t.routerLink===e)}getBlogContentByUrl(e){let t=e.split("/"),a=t.at(-2)??"",s=t.at(-1)??"";return g.find(r=>r.category===a&&r.id===+s)}getBlogContent(e,t){return g.find(a=>a.category===e&&a.id===t)}static \u0275fac=function(t){return new(t||l)(k(y))};static \u0275prov=b({token:l,factory:l.\u0275fac,providedIn:"root"})};export{M as a,v as b,f as c,w as d,g as e,F as f};
