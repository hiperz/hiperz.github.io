import{a as e}from"./chunk-L3UYHT7M.js";var t="pbrFragmentExtraDeclaration",g=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=g);var T={name:t,shader:g};var o="subSurfaceScatteringFunctions",u=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=u);var O={name:o,shader:u};var i="importanceSampling",E=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=E);var R={name:i,shader:E};var a="pbrHelperFunctions",h=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=h);var U={name:a,shader:h};var r="pbrDirectLightingSetupFunctions",p=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=p);var x={name:r,shader:p};var n="pbrDirectLightingFalloffFunctions",L=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=L);var W={name:n,shader:L};var l="hdrFilteringFunctions",D=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=D);var q={name:l,shader:D};var f="pbrDirectLightingFunctions",m=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[f]||(e.IncludesShadersStore[f]=m);var k={name:f,shader:m};var c="pbrBlockNormalGeometric",v=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[c]||(e.IncludesShadersStore[c]=v);var j={name:c,shader:v};var s="pbrBlockImageProcessing",A=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=A);var $={name:s,shader:A};var d="pbrDebug",C=`#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[d]||(e.IncludesShadersStore[d]=C);var oe={name:d,shader:C};export{T as a,O as b,R as c,U as d,x as e,W as f,q as g,k as h,j as i,$ as j,oe as k};
