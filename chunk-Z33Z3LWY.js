import{k as y}from"./chunk-N4EBHNUV.js";import{Lc as w,X as b,a as h,aa as f,b as g,p as u}from"./chunk-E5GNSJYM.js";var m=[{category:"csharp",id:16,content:`<h2>Table of Contents</h2>\r
<ul>\r
    <li><a href="#install">Installing NLog.Web.AspNetCore</a></li>\r
    <li><a href="#program">Configuring Program.cs</a></li>\r
    <li><a href="#config">Creating NLog.config</a></li>\r
    <li><a href="#usage">Using ILogger with NLog</a></li>\r
</ul>\r
\r
<h2 id="install">Installing NLog.Web.AspNetCore</h2>\r
<p>Search for <code>NLog.Web.AspNetCore</code> in the NuGet Package Manager and install it.</p>\r
<p><img src="./assets/images/articles/csharp/16/qdfa3_btrWwJ1x34a_1frXOgmuNjAk4TSGgB91t0_img.png" alt="NuGet Package Manager"></p>\r
<p>Alternatively, add the following line to your project file:</p>\r
<pre><code>&lt;PackageReference Include="NLog.Web.AspNetCore" Version="5.2.1" /&gt;</code></pre>\r
<br>\r
\r
<h2 id="program">Configuring Program.cs</h2>\r
<p>Add the following code to <code>Program.cs</code>:</p>\r
<pre><code>using NLog.Web;\r
\r
...\r
\r
// NLog\r
builder.Logging.ClearProviders();\r
builder.Logging.SetMinimumLevel(LogLevel.Trace);\r
builder.Host.UseNLog();\r
\r
...\r
\r
var app = builder.Build();</code></pre>\r
<br>\r
\r
<h2 id="config">Creating NLog.config</h2>\r
<p>Create an <code>NLog.config</code> file with the following content:</p>\r
<pre><code>&lt;?xml version="1.0" encoding="utf-8" ?&gt;\r
&lt;nlog xmlns="http://www.nlog-project.org/schemas/NLog.xsd"\r
      xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"\r
      autoReload="true"\r
      internalLogLevel="Info"\r
      internalLogFile="c:\\temp\\internal-nlog-AspNetCore.txt"&gt;\r
\r
    &lt;variable name="logFilePath" value="./Logs/MyApp.\${shortdate}.log" /&gt;\r
    \r
    &lt;extensions&gt;\r
        &lt;add assembly="NLog.Web.AspNetCore"/&gt;\r
    &lt;/extensions&gt;\r
\r
    &lt;targets&gt;\r
        &lt;target xsi:type="File"\r
                name="logfile"\r
                fileName="\${logFilePath}"\r
                layout="\${longdate} - LEVEL=\${level:upperCase=true}: \${message}"\r
                keepFileOpen="true"\r
                encoding="utf-8"\r
                archiveAboveSize="10000000"\r
                archiveNumbering="DateAndSequence"\r
                archiveDateFormat="yyyy-MM-dd" /&gt;\r
\r
        &lt;target xsi:type="Console" \r
                name="console" \r
                layout="\${longdate} - LEVEL=\${level:upperCase=true}: \${message}" /&gt;\r
    &lt;/targets&gt;\r
\r
    &lt;rules&gt;\r
        &lt;logger name="*" minlevel="Debug" writeTo="logfile" /&gt;\r
        &lt;logger name="*" minlevel="Debug" writeTo="console" /&gt;\r
    &lt;/rules&gt;\r
\r
&lt;/nlog&gt;</code></pre>\r
<br>\r
\r
<h2 id="usage">Using ILogger with NLog</h2>\r
<p>Once configured, all logs using the <code>ILogger</code> interface will be redirected to NLog. You can use either NLog's logging functions or the <code>ILogger</code> interface methods.</p>\r
<pre><code>public ILogger Logger { get; }\r
public NLog.Logger logger = NLog.LogManager.GetCurrentClassLogger();\r
\r
protected override async Task ExecuteAsync(CancellationToken stoppingToken)\r
{\r
    Logger.LogInformation("Log message from ILogger");\r
    logger.Info("Log message from NLog");\r
}</code></pre>\r
<p><img src="./assets/images/articles/csharp/16/yeKSK_btrWsVa1pmq_3cA32Q0xk9Q3WeiwI5dPjk_img.png" alt="Logging Example"></p>\r
<br>\r
`},{category:"csharp",id:17,content:`<h2>Table of Contents</h2>\r
<ul>\r
    <li><a href="#introduction">Introduction</a></li>\r
    <li><a href="#accessing-json-values">Accessing JSON Values</a></li>\r
    <li><a href="#mapping-to-class">Mapping Configuration to a Class</a></li>\r
    <li><a href="#benefits-of-mapping">Benefits of Using Encapsulated Instances</a></li>\r
</ul><br>\r
\r
<h2 id="introduction">Introduction</h2>\r
<p>When developing with ASP.NET, the WebApplication builder reads configuration files and provides access via the <code>ConfigurationManager</code> class. Typically, developers do not manually read and deserialize JSON configuration files into custom classes.</p>\r
<p>For small configuration files, direct access is manageable. However, when configurations become more complex with multiple properties, mapping them to a class improves maintainability and readability.</p><br>\r
\r
<h2 id="accessing-json-values">Accessing JSON Values</h2>\r
<p>Consider the following <code>appsettings.json</code> file:</p>\r
<pre class="javascript" data-ke-type="codeblock" data-ke-language="javascript"><code>{\r
  "Logging": {\r
    "LogLevel": {\r
      "Default": "Information",\r
      "Microsoft.AspNetCore": "Warning"\r
    }\r
  },\r
  "AllowedHosts": "*",\r
  "MyConfig": {\r
...
<p>To retrieve the values for <code>Host</code> and <code>Port</code>, you would use the following code:</p>\r
<pre class="javascript" data-ke-type="codeblock" data-ke-language="javascript"><code>var host = builder.Configuration["MyConfig:Host"];\r
var port = int.Parse(builder.Configuration["MyConfig:Port"]);</code></pre><br>\r
\r
<h2 id="mapping-to-class">Mapping Configuration to a Class</h2>\r
<p>Instead of accessing individual values directly, you can define a class and map the configuration section to an instance of that class:</p>\r
<pre class="javascript" data-ke-type="codeblock" data-ke-language="javascript"><code>public class MyConfig\r
{\r
    public string Host { get; set; }\r
    public int Port { get; set; }\r
}</code></pre>\r
<p>Then, retrieve the configuration as an instance of <code>MyConfig</code>:</p>\r
<pre class="javascript" data-ke-type="codeblock" data-ke-language="javascript"><code>using Newtonsoft.Json.Linq;\r
\r
var myconfig = builder.Configuration.GetSection("MyConfig").Get&lt;MyConfig&gt;();\r
\r
var host = myconfig.Host;\r
var port = myconfig.Port;</code></pre><br>\r
\r
<h2 id="benefits-of-mapping">Benefits of Using Encapsulated Instances</h2>\r
<p>When dealing with configurations that have numerous properties or nested structures, using a mapped class simplifies management and improves maintainability.</p><br>\r
`},{category:"csharp",id:19,content:`<h3>Table of Contents</h3>\r
<ul>\r
    <li><a href="#method1">Method 1: Configure in appsettings.json</a></li>\r
    <li><a href="#method2">Method 2: Define in Program.cs</a></li>\r
    <li><a href="#reference">Further Reading</a></li>\r
</ul>\r
<br>\r
\r
<h3 id="method1"><b>Method 1: Configure in appsettings.json</b></h3>\r
<p>Add the following configuration to your <code>appsettings.json</code> file to define Kestrel endpoints:</p>\r
...
<br>\r
\r
<h3 id="method2"><b>Method 2: Define in Program.cs</b></h3>\r
<p>You can also specify the endpoint directly in <code>Program.cs</code>:</p>\r
<pre class="csharp"><code>await app.RunAsync("http://localhost:5000");</code></pre>\r
<br>\r
\r
<h3 id="reference"><b>Further Reading</b></h3>\r
<p>For more details, refer to the official Microsoft documentation:</p>\r
<p><a href="https://learn.microsoft.com/en-us/aspnet/core/fundamentals/servers/kestrel/endpoints?view=aspnetcore-7.0" target="_blank" rel="noopener">Configure Endpoints in ASP.NET Core Kestrel Web Server</a></p>\r
<br>\r
`},{category:"csharp",id:21,content:`<h3>Table of Contents</h3>\r
<ul>\r
  <li><a href="#setup-static-files">Setting Up Static File Routing</a></li>\r
  <li><a href="#important-order">The Importance of Order</a></li>\r
</ul>\r
\r
<h3 id="setup-static-files">Setting Up Static File Routing</h3>\r
<p>When creating a WebAPI project using the default template, you might want to serve static HTML files directly from the base URL. To achieve this, add <code>UseDefaultFiles()</code> and <code>UseStaticFiles()</code> in the correct order, as shown below:</p>\r
<pre id="code_1674015460553" class="csharp" data-ke-language="csharp" data-ke-type="codeblock"><code>app.UseDefaultFiles();\r
app.UseStaticFiles();</code></pre>\r
<br>\r
\r
<h3 id="important-order">The Importance of Order</h3>\r
<p>It is crucial to maintain the correct order of these middleware calls. If <code>UseStaticFiles()</code> is placed before <code>UseDefaultFiles()</code>, entering the base service URL in the browser will not automatically route to <code>index.html</code>. Instead, a "Not Found" error will be returned.</p>\r
<br>\r
`},{category:"csharp",id:22,content:`<h2>Table of Contents</h2>\r
<ul>\r
  <li><a href="#introduction">Introduction</a></li>\r
  <li><a href="#problem">The Problem</a></li>\r
  <li><a href="#solution">The Solution</a></li>\r
</ul>\r
\r
<h2 id="introduction">Introduction</h2>\r
<p>When creating an Angular or React frontend for an ASP.NET Core WebAPI project, the production build static files need to be included in the WebAPI project. To ensure these files are copied to the output or publish directory during the WebAPI build process, the project file must be configured appropriately.</p><br>\r
\r
<h2 id="problem">The Problem</h2>\r
<p>One approach is to manually set the properties of each file in the <code>wwwroot</code> folder using Visual Studio. However, this method is cumbersome, as it requires reconfiguration whenever new files are added or renamed within the <code>wwwroot</code> folder.</p><br>\r
\r
<h2 id="solution">The Solution</h2>\r
<p>A more efficient approach is to modify the project file by adding a single line, ensuring all files in the <code>wwwroot</code> folder are always copied to the output and publish directories.</p><br>\r
\r
<pre id="code_1674016147627" class="html xml" data-ke-language="html" data-ke-type="codeblock">\r
<code>&lt;ItemGroup&gt;\r
  &lt;Content Update="wwwroot\\**\\*" CopyToOutputDirectory="Always" CopyToPublishDirectory="Always" /&gt;\r
&lt;/ItemGroup&gt;</code>\r
</pre><br>\r
`},{category:"csharp",id:23,content:`<h2>Table of Contents</h2>\r
<ul>\r
...
`},{category:"csharp",id:25,content:`<p>When starting a WebAPI project, there are times when you need to allow all origins for testing purposes. You can achieve this by adding a simple CORS policy named <code>corsapp</code> and calling <code>app.UseCors</code> after building the application, as shown below:</p><br>\r
\r
<pre id="code_1674627756825" class="csharp" data-ke-language="csharp" data-ke-type="codeblock"><code>builder.Services.AddCors(p =&gt; p.AddPolicy("corsapp", builder =&gt;\r
{\r
    builder.WithOrigins("*").AllowAnyMethod().AllowAnyHeader();\r
}));\r
\r
var app = builder.Build();\r
\r
app.UseCors("corsapp");</code></pre><br>\r
`},{category:"csharp",id:26,content:`<h3>Table of Contents</h3>\r
<ul>\r
    <li><a href="#default-behavior">Default Behavior in ASP.NET</a></li>\r
    <li><a href="#disable-camel-case">Disabling Camel Case Formatting</a></li>\r
</ul>\r
\r
<h3 id="default-behavior">Default Behavior in ASP.NET</h3>\r
<p>By default, when calling a Web API endpoint in ASP.NET, the JSON response properties are formatted in camel case. \r
However, in C#, public class members are typically named using Pascal Case. \r
If you want to keep Pascal Case in the JSON response, you can disable the default camel case transformation.<br></p>\r
\r
<h3 id="disable-camel-case">Disabling Camel Case Formatting</h3>\r
<p>To disable camel case formatting in JSON responses, add the following code to the <code>Program.cs</code> file:<br></p>\r
\r
<pre id="code_1674628941968" class="csharp" data-ke-language="csharp" data-ke-type="codeblock"><code>builder.Services\r
    .AddControllersWithViews()\r
    .AddJsonOptions(opt => opt.JsonSerializerOptions.PropertyNamingPolicy = null);</code></pre><br>\r
`},{category:"csharp",id:28,content:`<h2>Table of Contents</h2>\r
<ul>\r
    <li><a href="#overview">Overview</a></li>\r
    <li><a href="#implementation">Implementation</a></li>\r
</ul>\r
\r
<h2 id="overview">Overview</h2>\r
<p>To enable JWT authentication in Swagger for your .NET application, add the following code to your project.</p><br>\r
\r
<h2 id="implementation">Implementation</h2>\r
<p>Add the following code to configure Swagger with JWT authentication:</p>\r
<pre id="code_1674793225455" class="csharp" data-ke-language="csharp" data-ke-type="codeblock">\r
<code>builder.Services.AddSwaggerGen(c =&gt;\r
{\r
    c.AddSecurityDefinition("Bearer", new OpenApiSecurityScheme()\r
    {\r
...
\r
    c.AddSecurityRequirement(new OpenApiSecurityRequirement\r
    {\r
        {\r
...
`},{category:"csharp",id:36,content:`<h2>Table of Contents</h2>\r
<ul>\r
    <li><a href="#error-occurrence">Error Occurrence</a></li>\r
    <li><a href="#checking-event-log">Checking the Event Log</a></li>\r
    <li><a href="#fixing-the-issue">Fixing the Issue</a></li>\r
</ul>\r
\r
<h2 id="error-occurrence">Error Occurrence</h2>\r
<p>If you encounter an error after building an ASP.NET Core project in .NET 6, registering it as a Windows Service, and starting the service, follow these steps to diagnose and fix the issue.</p><br>\r
\r
<h2 id="checking-event-log">Checking the Event Log</h2>\r
<p>First, open the Event Log Viewer and check the exception message.</p>\r
<p><img src="./assets/images/articles/csharp/36/b42tZU_btrYJZBwWo4_bCjDsa0FDAVubsh3KEMyV1_img.png" alt="Event Log Viewer Error" style="display: block; margin: auto;"></p>\r
<p>Navigate to <strong>Windows Logs &rarr; Application</strong> and look for a <strong>.NET Runtime Error</strong>. This log typically reveals the exact location in the code where the exception occurred.</p><br>\r
\r
<h2 id="fixing-the-issue">Fixing the Issue</h2>\r
<p>If the error is caused by code accessing configuration settings from <code>appsettings.json</code>, you can resolve it by specifying the <code>ContentRootPath</code> when creating the application builder.</p>\r
\r
<pre class="csharp"><code>var builder = WebApplication.CreateBuilder(new WebApplicationOptions\r
{\r
    Args = args,\r
    ContentRootPath = WindowsServiceHelpers.IsWindowsService() ? AppContext.BaseDirectory : default,\r
});</code></pre>\r
\r
<p>For unknown reasons, when running the application as a Windows Service, the <code>ContentRootPath</code> may be set incorrectly. By explicitly setting it as shown above, the application should function properly.</p><br>\r
`},{category:"csharp",id:37,content:`<h2>Table of Contents</h2>\r
<ul>\r
    <li><a href="#introduction">Introduction</a></li>\r
    <li><a href="#problem">The Problem</a></li>\r
    <li><a href="#solution">The Solution</a></li>\r
...
\r
<h2 id="introduction">Introduction</h2>\r
<p>When developing WebAPI servers using ASP.NET Core, it's common to deploy an Angular front-end within the <code>wwwroot</code> folder. Often, the Angular app handles routing separately, leading to issues when users refresh the page.</p><br>\r
\r
<h2 id="problem">The Problem</h2>\r
<p>Since ASP.NET Core processes HTTP requests before Angular\u2019s routing takes effect, the server may not recognize client-side routes. As a result, refreshing a page within the Angular app results in a <code>404 Not Found</code> error.</p><br>\r
\r
<h2 id="solution">The Solution</h2>\r
<p>To resolve this, we can implement middleware that redirects unknown routes back to the root path (<code>/</code>), allowing Angular to handle routing properly.</p><br>\r
\r
<h2 id="code-example">Code Example</h2>\r
<pre class="csharp"><code>app.Use(async (context, next) =&gt;\r
{\r
    await next();\r
\r
    if (context.Response.StatusCode == 404)\r
    {\r
...
<br>`},{category:"csharp",id:77,content:`<p>When using OpenCVSharp to perform image processing in a WebAPI application and building a Docker container, calling the API results in a DllNotFoundException error during the OpenCVSharp function calls.</p>\r
\r
<p>This issue occurs because the container does not have the appropriate OpenCVSharp library binaries for the Linux kernel. To resolve this, you need to download the OpenCVSharp code, compile the library, and copy it to the /usr/lib/ directory by adding the following instructions to your Dockerfile:</p>\r
\r
<pre id="code_1691596797176" class="bash" data-ke-language="bash" data-ke-type="codeblock">\r
<code>ENV OPENCV_VERSION=4.8.0\r
\r
# Install opencv dependencies\r
RUN apt-get update && apt-get -y install --no-install-recommends \\\r
      ...
      libvorbis-dev \\\r
      libxvidcore-dev \\\r
      libopencore-amrnb-dev \\\r
      libopencore-amrwb-dev \\\r
      x264 \\\r
      libtesseract-dev \\\r
      libgdiplus \\\r
    && apt-get -y clean \\\r
...
\r
# Setup opencv and opencv-contrib source\r
RUN wget -q https://github.com/opencv/opencv/archive/\${OPENCV_VERSION}.zip && \\\r
    unzip -q \${OPENCV_VERSION}.zip && \\\r
    rm \${OPENCV_VERSION}.zip && \\\r
    mv opencv-\${OPENCV_VERSION} opencv && \\\r
    wget -q https://github.com/opencv/opencv_contrib/archive/\${OPENCV_VERSION}.zip && \\\r
    unzip -q \${OPENCV_VERSION}.zip && \\\r
    rm \${OPENCV_VERSION}.zip && \\\r
    mv opencv_contrib-\${OPENCV_VERSION} opencv_contrib\r
\r
# Build OpenCV\r
RUN cd opencv && mkdir build && cd build && \\\r
    cmake \\\r
    -D OPENCV_EXTRA_MODULES_PATH=/opencv_contrib/modules \\\r
    -D CMAKE_BUILD_TYPE=RELEASE \\\r
    -D BUILD_SHARED_LIBS=OFF \\\r
    -D ENABLE_CXX11=ON \\\r
    -D BUILD_EXAMPLES=OFF \\\r
    -D BUILD_DOCS=OFF \\\r
    -D BUILD_PERF_TESTS=OFF \\\r
    -D BUILD_TESTS=OFF \\\r
    -D BUILD_JAVA=OFF \\\r
    -D BUILD_opencv_app=OFF \\\r
    -D BUILD_opencv_barcode=OFF \\\r
    -D BUILD_opencv_java_bindings_generator=OFF \\\r
    -D BUILD_opencv_js_bindings_generator=OFF \\\r
    -D BUILD_opencv_python_bindings_generator=OFF \\\r
    -D BUILD_opencv_python_tests=OFF \\\r
    -D BUILD_opencv_ts=OFF \\\r
    -D BUILD_opencv_js=OFF \\\r
    -D BUILD_opencv_bioinspired=OFF \\\r
    -D BUILD_opencv_ccalib=OFF \\\r
    -D BUILD_opencv_datasets=OFF \\\r
    -D BUILD_opencv_dnn_objdetect=OFF \\\r
    -D BUILD_opencv_dpm=OFF \\\r
    -D BUILD_opencv_fuzzy=OFF \\\r
    -D BUILD_opencv_gapi=OFF \\\r
    -D BUILD_opencv_intensity_transform=OFF \\\r
    -D BUILD_opencv_mcc=OFF \\\r
    -D BUILD_opencv_objc_bindings_generator=OFF \\\r
    -D BUILD_opencv_rapid=OFF \\\r
    -D BUILD_opencv_reg=OFF \\\r
    -D BUILD_opencv_stereo=OFF \\\r
    -D BUILD_opencv_structured_light=OFF \\\r
    -D BUILD_opencv_surface_matching=OFF \\\r
    -D BUILD_opencv_videostab=OFF \\\r
    -D BUILD_opencv_wechat_qrcode=ON \\\r
    -D WITH_GSTREAMER=OFF \\\r
    -D WITH_ADE=OFF \\\r
    -D OPENCV_ENABLE_NONFREE=ON \\\r
    .. && make -j$(nproc) && make install && ldconfig\r
\r
# Download OpenCvSharp\r
RUN git clone https://github.com/shimat/opencvsharp.git && cd opencvsharp\r
\r
# Install the Extern lib.\r
RUN mkdir /opencvsharp/make && cd /opencvsharp/make && \\\r
    cmake -D CMAKE_INSTALL_PREFIX=/opencvsharp/make /opencvsharp/src && \\\r
    make -j$(nproc) && make install && \\\r
    rm -rf /opencv && \\\r
    rm -rf /opencv_contrib && \\\r
    cp /opencvsharp/make/OpenCvSharpExtern/libOpenCvSharpExtern.so /usr/lib/</code>\r
</pre>\r
\r
<p>Once you add the commands above and build and run the Docker container, the OpenCVSharp functions should work correctly.</p>\r
\r
<p>Source: <a href="https://github.com/khuyen94/OpenCVSharp-dotnet6-docker/blob/main/OpenCVSharp-dotnet6-docker/Dockerfile" target="_blank" rel="noopener">https://github.com/khuyen94/OpenCVSharp-dotnet6-docker/blob/main/OpenCVSharp-dotnet6-docker/Dockerfile</a></p>\r
\r
<p>Alternatively, you can resolve the issue without modifying the Dockerfile by using ImageSharp or SkiaSharp instead of OpenCVSharp.</p>\r
\r
<p>ImageSharp has become a paid library starting from version 3.x, so for commercial applications, you will need to use version 2.x (Apache 2.0 license).</p>\r
\r
<p>SkiaSharp, however, has a limitation where it cannot process 24-bit images, which could be an issue if you need to manipulate pixel data. Additionally, when using ImageSharp in WPF applications, you may encounter performance (speed) issues when converting to BitmapSource for display purposes.</p><br>\r
